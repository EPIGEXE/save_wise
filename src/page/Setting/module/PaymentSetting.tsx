...
import { IAsset, IPaymentMethod } from "@/types";
...
const PaymentSetting = () => {
    // ========================================== 상태 정의 ==========================================
    const [currentPaymentMethod, setCurrentPaymentMethod] = useState<IPaymentMethod>({
        id: 0,
        name: "",
        type: "cash",
        paymentDay: null,
    ...
    const [paymentMethods, setPaymentMethods] = useState<IPaymentMethod[]>([]); // 결제 방법 목록
    const [assets, setAssets] = useState<IAsset[]>([]); // 자산 목록

    const [isSheetOpen, setIsSheetOpen] = useState(false); // 결제 방법 추가 다이얼로그 열림 여부
    const [isEditMode, setIsEditMode] = useState(false); // 결제 방법 수정 모드 여부
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const submittedData = {
                ...
                paymentDay: currentPaymentMethod.type === "credit" ? currentPaymentMethod.paymentDay || null : null,
            };
            ...
            <Dialog open={isSheetOpen} onOpenChange={handleCloseModal}>
                <DialogContent>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <Select value={currentPaymentMethod.type} onValueChange={handleTypeChange}>
                            <SelectTrigger className="mb-2">
                        ...
                        >
                            <SelectTrigger
                                className={`mb-2 ${currentPaymentMethod.type !== "credit" ? "bg-red-100" : ""}`}
                            >
