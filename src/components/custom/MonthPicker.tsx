...
export function MonthPicker({ value, onValueChange }: { value: Date; onValueChange: (date: Date) => void }) {
    // ========================================== 상태 정의 ==========================================
    const [showMonths, setShowMonths] = useState(false); // 월 선택 표시 여부
    const [popoverOpen, setPopoverOpen] = useState(false); // 팝오버 열림 여부
    const [viewDate, setViewDate] = useState(new Date()); // 보고 있는 날짜

    const [date, setDate] = useState<Date>(value); // 선택된 날짜
    const [selectedYear, setSelectedYear] = useState<number>(); // 선택된 연도
                ...
                {!showMonths ? (
                    <div className="space-y-2">
                        <div className="flex items-center justify-between">
                            <Button variant="outline" size="icon" onClick={handlePreviousYears}>
                                <ChevronLeft className="h-4 w-4" />
                            </Button>
                            <div className="font-semibold">
                                {years[0]} - {years[years.length - 1]}
                            </div>
                            <Button variant="outline" size="icon" onClick={handleNextYears}>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            <Button variant="outline" onClick={() => setShowMonths(false)}>
                                ← {selectedYear}년
                            </Button>
                        </div>
                        <div className="grid grid-cols-3 gap-2">
                            {months.map((month) => (
                                <Button
                                    key={month}
                                    variant={
                                        date && date.getFullYear() === selectedYear && date.getMonth() === month
                                            ? "default"
                                            : "outline"
                                    }
